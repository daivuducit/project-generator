  ...
  public async createProject(context: vscode.ExtensionContext): Promise<void> {
    ...
    try {
      ...
      const cookiecutterConfig = JSON.parse(
        fs.readFileSync(
          path.join(finalTemplatePath, "cookiecutter.json"),
          "utf8",
        ),
      );
      ...
      await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          // Create a temporary config file with the inputs
          const configPath = path.join(
            os.tmpdir(),
            `cookiecutter-config-${Date.now()}.json`,
          );
          const config = {
            cookiecutter: inputs,
          };
          ...
          const command = `cookiecutter "${finalTemplatePath}" --no-input -o "${outputDir}" -c "${configPath}"`;
